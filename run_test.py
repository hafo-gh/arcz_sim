...
def px4_mavlink_override_args(config: TestConfig, output_dir: Path, log_path: Path) -> list[str]:
    ...
    script = f"""#!/bin/sh
# Generated by arcz_sim for explicit QGroundControl unicast.
...
# GCS link: send directly to QGroundControl, which should listen on UDP {config.qgc_port}.
mavlink start -x -u $udp_gcs_port_local -r 4000000 -f -t {config.qgc_host} -o {config.qgc_port}
...
"""
    ...
    append_log(log_path, f"[runner] QGC MAVLink target: {config.qgc_host}:{config.qgc_port}")
